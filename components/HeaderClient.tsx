...
export function HeaderClient({ 
    ...
    return (
        <>
            {/* Mobile Menu Backdrop (Moved to bottom for correct layering) */}
            ...
            <header className="sticky top-0 z-[10000] w-full bg-white border-b border-black/10 text-black">
                <div className="mx-auto max-w-[1800px] px-4 md:px-6">
                    {/* Header Layout */}
                    <div className="flex h-16 md:h-20 items-center justify-between gap-2 md:gap-0">
                        
                        {/* Left Column: Logo & Catalog (Desktop) */}
                        <div className="md:flex-1 flex items-center gap-4 md:gap-8">
                            <Link
                                href="/"
                                className="text-xl md:text-2xl font-black tracking-tighter flex items-center gap-1 md:gap-2 flex-shrink-0"
                            >
                                <span className="bg-black text-white px-1.5 md:px-2.5 py-0.5 transform -skew-x-12 text-sm md:text-2xl">ANVI</span>
                                <span className="text-black text-sm md:text-2xl">BUD</span>
                            </Link>
                        ...
                        {/* Center Column: Search Bar (Adaptive) */}
                        <div className="flex-1 md:flex-[2] flex justify-center px-1 md:px-8">
                            <form onSubmit={handleSearch} className="relative group w-full max-w-4xl">
                                <input
                                    ...
                                    className="w-full bg-zinc-100 border-none pl-8 md:pl-14 pr-4 py-2 md:py-3.5 text-[12px] md:text-sm focus:ring-1 md:focus:ring-2 focus:ring-black/5 transition-all outline-none rounded-sm md:rounded-none"
                                />
                                <Search className="absolute left-2.5 md:left-5 top-1/2 -translate-y-1/2 text-black/30 group-focus-within:text-black transition-colors w-3.5 h-3.5 md:w-5 md:h-5" />
                                {searchQuery && (
                                    <button 
                                        type="button" 
                                        onClick={() => setSearchQuery("")}
                                        className="hidden md:block absolute right-5 top-1/2 -translate-y-1/2 text-black/30 hover:text-black transition-colors"
                                    >
                                        <X size={18} />
                                    </button>
                                )}
                        ...
                        <div className="md:flex-1 flex items-center justify-end gap-1 md:gap-6">
                            ...
                            <Link href="/wishlist" className="relative p-1.5 md:p-2 hover:bg-zinc-100 transition-colors rounded-full group" aria-label="Wishlist">
                                <Heart size={18} className="text-black md:w-[22px] md:h-[22px] group-hover:fill-black transition-all" />
                                {effectiveWishlistCount > 0 && (
                                    <span className="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-[8px] md:text-[9px] font-bold w-3.5 h-3.5 md:w-4 md:h-4 flex items-center justify-center rounded-full">{effectiveWishlistCount}</span>
                                )}
                            </Link>

                            <Link href="/cart" className="relative p-1.5 md:p-2 hover:bg-zinc-100 transition-colors rounded-full group" aria-label="Cart">
                                <ShoppingBag size={18} className="text-black md:w-[22px] md:h-[22px]" />
                                {effectiveCartCount > 0 && (
                                    <span className="absolute -top-0.5 -right-0.5 bg-black text-white text-[8px] md:text-[9px] font-bold w-3.5 h-3.5 md:w-4 md:h-4 flex items-center justify-center rounded-full">{effectiveCartCount}</span>
                                )}
                            </Link>

                            <button
                                onClick={() => setIsMobileMenuOpen(true)}
                                className="md:hidden p-1.5 hover:bg-black/5 rounded-full"
                            >
                                <Menu size={20} />
                            </button>
                        </div>
            ...
            {/* Mobile Menu Backdrop (Above header) */}
            <AnimatePresence>
                {isMobileMenuOpen && (
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100001] md:hidden"
                        onClick={() => setIsMobileMenuOpen(false)}
                    />
                )}
            </AnimatePresence>

            {/* Desktop Catalog Backdrop (Below header) */}
            <AnimatePresence>
                {isCatalogOpen && (
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 bg-black/20 backdrop-blur-[2px] z-[9999] hidden md:block"
            ...
            <AnimatePresence>
                {isMobileMenuOpen && (
                    <motion.div
                        ...
                        className="fixed inset-y-0 left-0 w-[85%] max-w-sm bg-white z-[100002] shadow-2xl flex flex-col"
                    >
                                ...
                                {!mobileActiveCategory ? (
                                        ...
                                        <div className="space-y-4">
                                            {categories.map((category) => (
                                                <button
                                                    ...
                                                    className="w-full flex items-center justify-between text-lg font-black uppercase italic tracking-tighter text-left group"
                                                >
                                    ...
                                    >
                                        <button 
                                            ...
                                            className="flex items-center gap-2 text-[10px] uppercase font-black tracking-widest text-zinc-400 hover:text-black transition-colors"
                                        >
                                        ...
                                        <div className="space-y-6">
                                            <div className="space-y-2">
                                                <h3 className="text-2xl font-black uppercase italic tracking-tighter">
                                                    {getLocalized(mobileActiveCategory)}
                                                </h3>
                                                <Link 
                                                    href={`/${mobileActiveCategory.slug}`}
                                                    onClick={() => setIsMobileMenuOpen(false)}
                                                    className="inline-block text-[10px] font-bold uppercase tracking-widest border-b border-black pb-0.5"
                                                >
                                                    Дивитись всі товари
                                                </Link>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-1 gap-4">
                                                {mobileActiveCategory.subcategories.map((sub) => (
                                                    <Link
                                                        key={sub.id}
                                                        href={`/${mobileActiveCategory.slug}/${sub.slug}`}
                                                        onClick={() => setIsMobileMenuOpen(false)}
                                                        className="flex items-center gap-4 group"
                                                    >
                                                        <div className="w-12 h-12 bg-zinc-100 flex-shrink-0 flex items-center justify-center overflow-hidden border border-black/5">
                                                            {sub.image ? (
                                                                <img src={sub.image} alt="" className="w-full h-full object-cover" />
                                                            ) : (
                                                                <LayoutGrid size={16} className="text-black/10" />
                                                            )}
                                                        </div>
                                                        <span className="text-sm font-bold uppercase tracking-tight text-zinc-600 group-active:text-black">
                                                            {getLocalized(sub)}
                                                        </span>
                                                    </Link>
                                                ))}
